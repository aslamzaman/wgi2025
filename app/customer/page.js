...
const Customer = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                const data = await getDataFromFirebase("customer");
                            ...
                            {customers.length ? (
                                customers.map(customer => (
                                    <tr className="border-b border-gray-200 hover:bg-gray-100" key={customer.id}>  
                                        <td className="text-center py-1 px-4">{customer.name}</td>
                                        <td className="text-center py-1 px-4">{customer.address}</td>
                                        <td className="text-center py-1 px-4">{customer.contact}</td>                                      
