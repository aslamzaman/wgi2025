...
const Details = ({ message, id, data }) => {
    ...
    const showDetailsForm = async () => {
        ...
        try {
            ...
            const sortedSale = sale.sort((a, b) => sortArray(new Date(a.dt), new Date(b.dt)));
            ...
            const sortePayment = payment.sort((a, b) => sortArray(new Date(a.dt), new Date(b.dt)));

            setPayments(sortePayment);
                                        ...
                                        payments.map((payment, i) => {
                                            ...
                                            return (
                                                <tr className={`border-b border-gray-200 hover:bg-gray-100`} key={payment.id}>
                                                    <td className="text-start py-2 px-4">{i + 1}</td>
                                                    <td className="text-start py-2 px-4">{formatedDateDot(payment.dt,true)}</td>
